...
namespace NeuralNetworkAPI.Data.Networks
{
    public class NetworkManager
    {
        ...
        public static List<bool> Process(NetworkMetadata metadata, NetworkInput input)
        {
            Network network = Network.Load(ParseSaveFile(metadata));
            return network.Process(new NetworkData(input.Inputs)).BitValues.RawValues;
        }

        public static List<bool> Teach(NetworkMetadata metadata, NetworkInput input)
        {
            Network network = Network.Load(ParseSaveFile(metadata));
            input.TeachRepeats = input.TeachRepeats > 0 ? input.TeachRepeats : 1;
            List<LearningData> learningData = new List<LearningData>() { new LearningData(new NetworkData(input.Inputs), new NetworkData(input.ExpectedOutputs)) };
        ...
        public static float Teach(NetworkMetadata metadata, LearningDataInput input)
        {
            ...
            List<LearningData> learningData = input.Cases.Select(x => new LearningData(new NetworkData(x.Inputs), new NetworkData(x.ExpectedOutputs))).ToList();
